...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    const response = await fetch(
      "http://localhost:3000/api/admin/rewards/add",
      {
        method: "POST",
        headers: {
    ...
    if (data.success) {
      const messageBox = document.getElementById("messageBoxadmin");
      messageBox.style.display = "block";
      messageBox.textContent = data.message;
      document.getElementById("addCouponForm").reset();
      document.getElementById("addCouponForm").style.display = "none";
      document.getElementById("showAddCouponFormBtn").textContent =
        "+ Add Reward";
      setTimeout(() => {
