      ...
      card.innerHTML = `
       ...
       <input type="hidden" name="product_id" value="${p.id}">
        ...
        <button type="submit">Apply Coupon</button>
           ...
           <p>
               <b>Original Price:</b> <span id="original-price-${
                 p.id
          ...
          <div id="messageBox-${
            p.id
          }" class="alert" style="display:block"></div>
...
function setupCouponListeners() {
  ...
  applyForms.forEach((form) => {
    form.addEventListener("submit", async (e) => {
      ...
      const product_id = e.currentTarget.querySelector(
        'input[name="product_id"]'
      ).value;
      const price = e.currentTarget.querySelector('input[name="price"]').value;
      const coupon_code = e.currentTarget.querySelector(
        'input[name="coupon_code"]'
      ).value;

      const res = await fetch(
        "http://localhost:3000/api/userDashboard/apply-coupon",
        {
          method: "POST",
      ...
      const discountDetailsArea = document.getElementById(
        `discount-details-${product_id}`
      );
      const originalPriceDisplay = document.getElementById(
        `original-price-${product_id}`
      );
    ...
    `;
        ...
        const finalPriceInput = document.getElementById(
          `final_price-${product_id}`
        );
        const amountSavedInput = document.getElementById(
          `amountSaved-${product_id}`
        );
        const originalPriceInput = document.getElementById(
          `originalPrice-${product_id}`
        );

        if (!discountInput || !finalPriceInput) {
          console.error(
            `[CRITICAL ERROR] Product ID ${product_id}: Cannot find ALL hidden inputs.`
          );
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        let originalPrice = Number(
          document.getElementById(`originalPrice-${productId}`).value
        );
        let finalPrice = Number(
          document.getElementById(`final_price-${productId}`).value
        );
        let discount = Number(
          document.getElementById(`discount-${productId}`).value
        );
        let amountSaved = Number(
          document.getElementById(`amountSaved-${productId}`).value
        );

        // Log the final values read from the form *before* sending them
        console.log("Data read from Buy Form (Final Check):", {
          productId,
          finalPrice,
          discount,
          originalPrice,
        ...
        if (!finalPrice || isNaN(finalPrice) || finalPrice === 0) {
          console.log(
            "No valid final price read, using original price as fallback."
          );
        ...
        if (data.success && data.redirectTo) {
          ...
          alert(
            `Checkout failed: ${data.message || "An unknown error occurred."}`
          );
        }
      } catch (err) {
