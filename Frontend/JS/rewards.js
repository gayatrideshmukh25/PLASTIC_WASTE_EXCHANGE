...
async function redeemCoupon(id) {
  const res = await fetch("http://localhost:3000/api/userDashboard/rewards/redeem", {
    method: "POST",
  ...
  if (data.success) {
    ...
    messageBox.innerHTML = `<div class="alert" style="background:#ffe6e6;">${data.message || "Failed to redeem coupon."}</div>`;
  }
}
