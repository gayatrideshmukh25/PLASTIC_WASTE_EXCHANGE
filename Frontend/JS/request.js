...
document.getElementById("findLocationBtn").addEventListener("click", () => {
  ...
  function success(position) {
    ...
    locationStatus.textContent = `Your location: (${latitude.toFixed(
      4
    )}, ${longitude.toFixed(4)})`;
      ...
      .then((data) => {
        if (data.name) {
          ...
          nearestDiv.textContent =
            data.message || "No nearby collectors found.";
        }
      })
      .catch(() => {
...
document.getElementById("wasteForm").addEventListener("submit", async (e) => {
  ...
  const collector_id = document.querySelector(
    "input[name='collector_id']"
  ).value;
  const waste_type = document.querySelector("select[name='waste_type']").value;
  const quantity = document.querySelector("input[name='quantity']").value;
  const pickup_address = document.querySelector(
    "textarea[name='pickup_address']"
  ).value;
  const preferred_date = document.querySelector(
    "input[name='preferred_date']"
  ).value;
  const preferred_time = document.querySelector(
    "input[name='preferred_time']"
  ).value;
  const notes = document.querySelector("textarea[name='notes']").value;

  const res = await fetch(
    "http://localhost:3000/api/userDashboard/postRequest",
    {
      method: "POST",
