...
exports.postlogin = [
    ...
    try {
      ...
      if (!userData) {
        return res.json( {
          errors: [],
          errorMessage: "Email / password / userType incorrect",
          oldInput: { email },
        });
      }
    ...
    req.session.user = {id: userData.id, userType: userData.userType};
        req.session.save((err) => {
            ...
            if(userType == 'collector'){
                 return res.json({success:true,redirectTo:'/collectorDashboard.html'});
            }
            if(userType == 'admin'){
                 return res.json({success:true,redirectTo:'/adminDashboard.html'});
            }
      });
    }   catch(error){
...
exports.postsignup = [
  ...
  check('availablity')
    ...
    try {
      ...
      return res.json({success:true,redirectTo:'/login.html'});
    } catch (err) {
...
exports.logout = (req,resp,next) => {
    req.session.destroy((err) => {
        ...
        resp.json({success:true,redirectTo:'/login.html'});
    }
    )
}